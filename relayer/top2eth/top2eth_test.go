...
func TestSubmitHeader(t *testing.T) {
	...
	t.Log("header hexstring data:", hex.EncodeToString(data))

	dedata := common.Hex2Bytes(hex.EncodeToString(data))

	var debath []string
	err = base.DecodeHeaders(dedata, &debath)
	if err != nil {
		t.Fatal("DecodeHeaders:", err)
	}
	t.Log("decode data:", debath)
